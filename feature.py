...
# New function for shop items
def shop_items(monsters_killed):
    """
    Determines available items in the shop based on the number of monsters killed.
    """
    all_items = [
        {"name": "Basic Potion", "type": "potion", "rarity": "common", "price": 10},
        {"name": "Iron Sword", "type": "melee", "rarity": "uncommon", "price": 50},
        {"name": "Magic Wand", "type": "magic", "rarity": "rare", "price": 100},
        {"name": "Dragon Armor", "type": "armor", "rarity": "epic", "price": 500},
    ]

    if monsters_killed < 5:
        available_items = [item for item in all_items if item["rarity"] == "common"]
    elif monsters_killed < 10:
        available_items = [item for item in all_items if item["rarity"] in ["common", "uncommon"]]
    elif monsters_killed < 20:
        available_items = [item for item in all_items if item["rarity"] in ["common", "uncommon", "rare"]]
    else:
